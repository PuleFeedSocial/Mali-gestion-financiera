...
serve(async () => {
  ...
  const webpush = new WebPush({ vapid: { subject: `mailto:${vapidEmail}`, publicKey: vapidPublic, privateKey: vapidPrivate } })
  ...
  for (const sub of subs) {
    ...
    if (prefMap['debt_reminder'] !== false) {
      const { data: debts } = await supabase
        ...
        .eq('user_id', sub.user_id)
        .gt('installments_paid', 0)
        .lt('installments_paid', supabase.rpc('get_column', { table_name: 'debts', column_name: 'installments' }))
        .limit(5)

      if (debts && debts.length > 0) {
        notifications.push({
          title: `💳 ${debts.length} deuda(s) por pagar`,
          body: debts.slice(0, 3).map(d => d.name).join(', ') + (debts.length > 3 ? '...' : ''),
